...
def my_loss_fn(y_true, y_pred):
    # 下面是官方代码
    # 取自https://github.com/keras-team/keras/blob/6d7e23d98e516988e04455d93bde9a66d8e79566/keras/losses.py
    # y_pred = tf.convert_to_tensor(y_pred)
    # y_true = tf.cast(y_true, y_pred.dtype)
    # return backend.mean(tf.math.squared_difference(y_pred, y_true), axis=-1)

    # 尝试进行一些修正，对最终金额较低的预测减少权重
    #
    
    squared_difference = tf.square(y_true - y_pred)
    # 计算一个权重，设定一个线，最终超过这条线的按100%算，不足的按比例折算
    global line
    w = y_true
    w = w / line
    # w[w>1] = 1
    w = tf.where(tf.greater(w,1.0),1.0,w)
    squared_difference = squared_difference * w
...
def createModFunc1():
    ...
    mod.compile(optimizer='RMSprop',loss=my_loss_fn)
...
def train(dataDf3,message):
    ...
    for _ in range(5):
        for media in mediaList:
            ...
            testX, testY, testY0, testY1 = getTestingData(dataDf3,name,'2022-08-01','2022-08-30',mean,std)
...
# 带权重的mape
def wmapeFunc(y_true, y_pred):
    global line
    w = y_true.copy()
    w = w / line
    w[w>1] = 1
    return np.average(np.abs((y_pred - y_true) / y_true),weights = w) * 100
...
def createDoc(modPath,trainX,trainY0, trainY1,testX,testY0, testY1,history,docDirname,message):
    ...
    mod = tf.keras.models.load_model(modFilename,custom_objects={'my_loss_fn':my_loss_fn})
...
if __name__ == '__main__':
    ...
    train(df2,'mse media weight4 test')
