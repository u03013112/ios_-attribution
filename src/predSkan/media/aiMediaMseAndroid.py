...
def createModFunc2():
    mod = keras.Sequential(
        [
            ...
            # layers.Dropout(0.3),
            layers.Dense(64, kernel_initializer='random_normal',bias_initializer='random_normal',activation="relu"),
            # layers.Dropout(0.3),
            layers.Dense(1, kernel_initializer='random_normal',bias_initializer='random_normal',activation="relu")
        ]
    )
...
def train(dataDf3,message):
    ...
    earlyStoppingValLoss = tf.keras.callbacks.EarlyStopping(monitor='val_loss',patience=300)
    for _ in range(10):
        for media in mediaList:
            name = media['name']
            ...
            trainX,mean,std,trainY,trainY0,trainY1 = getTrainingData(dataDf3,name,'2022-08-01','2022-08-30')
...
if __name__ == '__main__':
    ...
    # df.to_csv('mediaA_20220501_20220930')
    # # df = pd.read_csv(getFilename('media_20220501_20220930'))
    # df1 = addMediaGroup(df)
    # df2 = dataFill(df1)
    # df2.to_csv(getFilename('mediaIdfaA_20220501_20220930'))

    df2 = pd.read_csv(getFilename('mediaIdfaA_20220501_20220930'))
    train(df2,'mse media fix train date2 android createModFunc1 without dropout')
