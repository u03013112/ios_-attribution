...
def getIdfaDataN(sinceTimeStr,unitlTimeStr,N = 1):
...
if __name__ == '__main__':
    if __debug__:
        ...
        for n in range(1,7):
            df = getIdfaDataN('20220501','20220930',N = n)
            df1 = addMediaGroup(df)
            df2 = dataFill(df1)
            df2.to_csv(getFilename('mediaIdfaN%d_20220501_20220930'%n))
    for n in range(1,7):
        df = pd.read_csv(getFilename('mediaIdfaN%d_20220501_20220930'%n))
        train(df,'mse media N=%d'%n)
