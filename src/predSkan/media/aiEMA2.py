...
def train(dataDf3):
    ...
    for media in mediaList:
        ...
        for n in [3,7,14,28]:
            ...
            checkpoint_filepath = '/src/src/predSkan/media/mod/modS%d_%s{epoch:05d}-{val_loss:.2f}.h5'%(n,name)
            ...
            trainDf = dataDf3.loc[
                (dataDf3.install_date >= '2022-09-01') & (dataDf3.install_date < '2022-09-15') & (dataDf3.media_group == name)
            ].sort_values(by=['install_date','cv'])    
            trainDf = trainDf.groupby(['install_date','cv']).agg('sum')
            
            trainXDf = dataDf3.loc[
                (dataDf3.install_date >= '2022-09-01') & (dataDf3.install_date < '2022-09-15') & (dataDf3.media_group == name)
            ].sort_values(by=['install_date','cv']).groupby(['install_date','cv']).agg('sum')
            trainX = trainXDf['count'].to_numpy().reshape((-1,64))
            
            trainSumByDay = trainDf.groupby('install_date').agg({'sumr1usd':'sum','sumr7usd':'sum'})
            trainY = trainSumByDay['sumr7usd'].to_numpy()
            # trainSumByDay = dataAddEMA(trainSumByDay,n)
            # print(trainSumByDay)
            # trainEMADf = trainSumByDay.loc[
            #     (trainSumByDay.install_date >= '2022-09-01') & (trainSumByDay.install_date < '2022-09-15')
            # ]
            # trainY = trainEMADf['ema'].to_numpy()

            testDf = dataDf3.loc[
                (dataDf3.install_date >= '2022-09-15') & (dataDf3.media_group == name)
            ].sort_values(by=['install_date','cv'])
...
if __name__ == '__main__':
    ...
    if __debug__:
        ...
        df2 = dataFill(df1)
        # df3 = addCvUsd(df2)
        df2.to_csv(getFilename('mediaIdfa2_20220501_20220930'))
