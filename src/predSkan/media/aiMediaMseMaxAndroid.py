...
def getIdfaDataMax(sinceTimeStr,unitlTimeStr,max = 200.0):
...
def train(dataDf3,message):
    ...
    for _ in range(10):
        for media in mediaList:
            ...
            checkpoint_filepath = '/src/src/predSkan/media/mod/std/mod%s_%s_{epoch:05d}-{loss:.2f}-{val_loss:.2f}.h5'%(name,filenameSuffix)
            ...
            lossAndErrorPrintingCallbackSuffixStr = media['name']

            trainX,mean,std,trainY,trainY0,trainY1 = getTrainingData(dataDf3,name,'2022-08-01','2022-09-01')
            testX, testY, testY0, testY1 = getTestingData(dataDf3,name,'2022-09-01','2022-10-01',mean,std)
            ...
            val_loss = createDoc('/src/src/predSkan/media/mod/std','mod%s'%name,trainX, trainY0,trainY1,testX,testY0, testY1,history,docDirname,message)
            # 将结果写入到国家日志里
...
if __name__ == '__main__':
    if __debug__:
        ...
        df = getIdfaDataMax('20220501','20220930')
        df1 = addMediaGroup(df)
        df2 = dataFill(df1)
        df2.to_csv(getFilename('mediaIdfaAMax200_20220501_20220930'))
        
    df = pd.read_csv(getFilename('mediaIdfaAMax200_20220501_20220930'))
    train(df,'mse max media android')
