...
def getTrainingData(df,mediaName,sinceTimeStr,unitlTimeStr,minUserCount=500,minPaidUserCount = 10):
    trainDf = df.loc[
    ...
    # 将人数太少的部分直接舍弃掉，没有预测价值，先定一个值，用户数少于500的直接剔除，既不做学习，也不做预测
    userCountNpArray = trainX.sum(axis=1)
    indexs = np.argwhere(userCountNpArray > minUserCount).reshape(-1)
    trainX = trainX[indexs]
    trainY0 = trainY0[indexs]
    trainY1 = trainY1[indexs]

    paidUserCountNpArray = trainX[:,1:].sum(axis=1)
    indexs = np.argwhere(paidUserCountNpArray > minPaidUserCount).reshape(-1)
    trainX = trainX[indexs]
    trainY0 = trainY0[indexs]
    trainY1 = trainY1[indexs]
...
def getTestingData(df,mediaName,sinceTimeStr,unitlTimeStr,mean,std,minUserCount=500,minPaidUserCount = 10):
    trainDf = df.loc[
        (df.install_date >= sinceTimeStr) & (df.install_date < unitlTimeStr) & (df.media_group == mediaName)
    ].sort_values(by=['install_date','cv'])
    trainDf = trainDf.groupby(['install_date','cv']).agg('sum')
...
def train(dataDf3,message,minUserCount=500,minPaidUserCount = 10):
    global lossAndErrorPrintingCallbackSuffixStr
    ...
    for _ in range(5):
        for media in mediaList:
            ...
            trainX,mean,std,trainY,trainY0,trainY1 = getTrainingData(dataDf3,name,'2022-05-01','2022-07-30',minUserCount,minPaidUserCount)
            if len(trainX) < 2:
                print ('minError:%s %d %d train %d!!!'%(name,minUserCount,minPaidUserCount,len(trainX)))
                continue

            testX, testY, testY0, testY1 = getTestingData(dataDf3,name,'2022-08-01','2022-09-01',mean,std,minUserCount,minPaidUserCount)
            if len(testX) < 2:
                print ('minError:%s %d %d test %d!!!'%(name,minUserCount,minPaidUserCount,len(testX)))
                continue
...
if __name__ == '__main__':
    ...
    for minUserCount in [200,500,1000]:
        for minPaidUserCount in [0,5,10,20]:
            train(df2,'mse media min %d %d'%(minUserCount,minPaidUserCount),minUserCount,minPaidUserCount)
