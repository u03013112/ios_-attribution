...
# 与retCheck.py的区别是，retCheck.py使用168小时作为7日回收，而retCheck2.py使用d6作为7日回收
...
def saveData():
    df = getDataFromMC()
    df.to_csv(getFilename('retCheckData_d6_20230101_20230401'))

def loadData():
    df = pd.read_csv(getFilename('retCheckData_d6_20230101_20230401'))
    return df
...
if __name__ == '__main__':
    ...
    print(cvMapDf)
    cvMapDf.to_csv(getFilename('cvMapRetCheckData_d6_20230101_20230401'))
    tmpDf = addCv(df,cvMapDf)
    df = df.merge(tmpDf,how='left',on='uid')
    # df.to_csv(getFilename('retCheckData_cv1_d6_20230101_20230401'),index=False)

    # df = pd.read_csv(getFilename('retCheckData_cv1_d6_20230101_20230401'))
    ...
    for cv1 in df['cv1'].unique():
        ...
        print(cv1MapDf)
        cv1MapDf.to_csv(getFilename('cvMapRetCheckData_d6_cv1_'+str(int(cv1))+'_d6_20230101_20230401'))

        tmpDf = tmpDf.append(addCv(cv1Df,cv1MapDf,cv='cv7',usd='hour168price',usdp='hour168priceP'))

    df = df.merge(tmpDf,how='left',on='uid') 
    df.to_csv(getFilename('retCheckData_cv1_cv7_d6_20230101_20230401'),index=False)


    # df = pd.read_csv(getFilename('retCheckData_cv1_cv7_d6_20230101_20230401'))
    ...
    # hour168price mape: 0.039757738480116565 r2: 0.9689292458911913

    # 计算预测结果的MAPE
    mape,r2 = check(df,usd='hour168price',usdp='predict_value')
    print('predict_value mape:',mape,'r2:',r2)
    # predict_value mape: 0.18705041884161477 r2: 0.6263866459172998
    ...
    for cv1 in df['cv1'].unique():
        ...
        cv1MapDf = pd.read_csv(getFilename('cvMapRetCheckData_d6_cv1_'+str(int(cv1))+'_d6_20230101_20230401'))
    ...
    print('predict_value mape:',mape,'r2:',r2)
    # predict_value mape: 0.16870055592120392 r2: 0.669428924191519
