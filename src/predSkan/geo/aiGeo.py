...
# 测试集仍用原始数据
# 结果存储目录结构大致如下
# geo/XX（比如：US）/存储所有美国的结果
# 该目录下有目前最好的5个结论的结论记录文件，应该记录有val loss + 对应doc路径
# 有每次测试的最佳结果记录文件，应该记录有val loss + 对应doc路径 + 备注（比如这个测试是为啥）
# doc目录，里面记录每次测试的具体情况，以供之后分析和追溯
...
def train(dataDf3,message):
    ...
    for _ in range(100):
        for geo in geoList:
            ...
            trainX, trainY = getTrainingData(dataDf3,name,'2022-08-01','2022-08-31')
...
if __name__ == '__main__':
    ...
    train(df4,'loop 100 times,for random args')
