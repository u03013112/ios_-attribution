# 尝试测试样本时间与结论是否有关
# 取之前一段时间做预测，试试看
# 这里的分歧可能是正好7月有一个大调整，这也是这项预测应该面对的困难
# 由于缩短了训练数据时间周期，对应的应该也缩短测试数据时间周期来对应，但是这里为了作对照测试，暂时保持测试数据时间周期不变
import pandas as pd
import os
import sys
sys.path.append('/src')
...
def getTrainingData(df,geoName,sinceTimeStr,unitlTimeStr):
    ...
    trainY = trainSumByDay['sumr7usd'].to_numpy()
    ...
    for geo in geoList:
        ...
        for n in [7,14,28,60]:
            ...
            for _ in range(10):
                ...
                sinceTime = datetime.datetime.strptime('2022-09-01','%Y-%m-%d')
                sinceTimeStr = (sinceTime-datetime.timedelta(days=n)).strftime('%Y-%m-%d')

                lossAndErrorPrintingCallbackSuffixStr = geo['name'] + sinceTimeStr

                trainX, trainY = getTrainingData(dataDf3,name,sinceTimeStr,'2022-08-31')
                testX, testY = getTrainingData(dataDf3,name,'2022-09-01','2022-09-30')
                ...
                val_loss = createDoc('/src/src/predSkan/geo/mod/std','mod%s'%name,trainX, trainY,testX,testY,history,docDirname,message)
                # 将结果写入到国家日志里
...
def createDoc(modPath,modSuffix,trainX,trainY,testX,testY,history,docDirname,message):
...
if __name__ == '__main__':
    ...
    train(df4,'change training data time range')
