...
import sys
sys.path.append('/src')
from src.predSkan.data import getTotalDataGroupByGeo,getTotalDataGroupByGeoMax2
...
def dataStep0(sinceTimeStr,unitlTimeStr,max):
    df = getTotalDataGroupByGeoMax2(sinceTimeStr,unitlTimeStr,max)
    df.to_csv(getFilename('totalGeoData%s_%s_%.2f'%(sinceTimeStr,unitlTimeStr,max)))

# 从本地文件取数据，跑过步骤0的可以直接从这里开始，更快速
def dataStep1(sinceTimeStr,unitlTimeStr,max):
    df = pd.read_csv(getFilename('totalGeoData%s_%s_%.2f'%(sinceTimeStr,unitlTimeStr,max)))
    return df
...
def train(dataDf3,message):
    ...
    for _ in range(10):
        for geo in geoList:
            ...
            trainX,mean,std,trainY,trainY0,trainY1 = getTrainingData(dataDf3,name,'2022-08-01','2022-08-31')
            testX, testY, testY0, testY1 = getTestingData(dataDf3,name,'2022-09-01','2022-09-30',mean,std)
...
if __name__ == '__main__':
    for max in [200,500]:
        dataStep0('20220501','20220930',max)
        df = dataStep1('20220501','20220930',max)
        df3 = dataStep3(df)
        df4 = dataStep4(df3)
        df4.to_csv(getFilename('geoData_20220501_20220930_%.2f'%max))
        df4 = pd.read_csv(getFilename('geoData_20220501_20220930_%.2f'%max))

        train(df4,'std args max2 %.2f'%max)
