# 方案四：此方案暂时只判断是否需要增加花费
# 与方案三的区别在于考虑高于kpi均值的幅度，如果高出幅度较大，就认为是较大的信号。
...
def func4( N = 30,p = 1.0):
    ...
    for name in nameList:
        ...
        if c > 0:
            ...
            # 超出幅度
            df.loc[df[name] > df['%s%d_kpi_mean'%(name,N)]*p,'over_kpi_%s'%name] = (df[name] - df['%s%d_kpi_mean'%(name,N)])/df['%s%d_kpi_mean'%(name,N)]+1
            df.loc[df[name] > df['%s%d_kpi_mean'%(name,N)]*p,'corr%s'%name] = c
            df.loc[df[name] > df['%s%d_kpi_mean'%(name,N)]*p,'increase_score'] += c*df[cWeightName]*df['over_kpi_%s'%name]
            ...
            df.loc[df[name] < df['%s%d_kpi_mean'%(name,N)]*p,'over_kpi_%s'%name] = (df['%s%d_kpi_mean'%(name,N)] - df[name])/df['%s%d_kpi_mean'%(name,N)]+1
            df.loc[df[name] < df['%s%d_kpi_mean'%(name,N)]*p,'corr%s'%name] = -c
            df.loc[df[name] < df['%s%d_kpi_mean'%(name,N)]*p,'increase_score'] -= c*df[cWeightName]*df['over_kpi_%s'%name]
    ...
    plt.tight_layout()
    plt.savefig('/src/data/ma_func4.png')
    print('save to /src/data/ma_func4.png')
    plt.clf()

    return df.loc[:,~df.columns.str.match('Unnamed')]

func4().to_csv(getFilename('func4'))
