# 与p2相似，差异是3测7，按照道理应该会更准
# 另外由于代码部分写死，3日回收字段仍用r1usd，这样代码的改变只有数据采集部分。
import pandas as pd
import numpy as np
import os
import sys
...
# 抄袭getAndroidData
def getiOSData(sinceTimeStr,unitlTimeStr):
...
# 抄袭getAndroidData2
def getiOSData2(sinceTimeStr,unitlTimeStr):
    whenStr = ''
...
def mergeData1AndData2(sinceTimeStr,unitlTimeStr):
    df = getiOSData(sinceTimeStr,unitlTimeStr)
    df2 = getiOSData2(sinceTimeStr,unitlTimeStr)
...
def createModFunc1():
    mod = keras.Sequential(
        [
            layers.Dense(256,kernel_initializer='random_normal',bias_initializer='random_normal', activation="relu", input_shape=(64,)),
            layers.Dropout(0.3),
            layers.Dense(256, kernel_initializer='random_normal',bias_initializer='random_normal',activation="relu"),
            layers.Dropout(0.3),
...
if __name__ == '__main__':
    if __debug__:
        ...
        df4 = dataStep3(df)
        df4.to_csv(getFilename('totalDataRt_3_20220701_20230201'))

    df4 = pd.read_csv(getFilename('totalDataRt_3_20220701_20230201'))
    train(df4,'3p7 func1 Rt 1911')