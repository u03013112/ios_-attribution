...
def createDoc(modPath,trainX,trainY0, trainY1,testX,testY0, testY1,history,docDirname,message):
    ...
    yp = (trainYP.reshape(-1)/20 + 3)*(trainY1.reshape(-1))
    pd.DataFrame(data={
    ...
    yt = testY0.reshape(-1)
    yp = (testYP.reshape(-1)/20 + 3)*(testY1.reshape(-1))
    pd.DataFrame(data={
