...
def dataStep1():
    ...
    df = df.groupby(['install_date','r1lp_cv'],as_index=False).agg({'r1usd':'sum','r7usd':'sum','count':'sum'})
    
    df = df.rename(columns={'r1usd':'sumr1usd'})
    df = df.rename(columns={'r7usd':'sumr7usd'})
    df = df.rename(columns={'r1lp_cv':'cv'})
...
if __name__ == '__main__':
    if __debug__:
        ...
        df4 = dataStep3(df)
        df4.to_csv(getFilename('totalData2_lp'))

    df4 = pd.read_csv(getFilename('totalData2_lp'))
    train(df4,'cv test lp')
