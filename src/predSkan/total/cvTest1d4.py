# 与test1的区别在于
# 目标不再是r7/r1-1，而是r7-r1*3

# 下面一行作废
# 其中输入不再是cv百分比分布，而是用cv数量分布
...
def dataStep1():
    ...
    df = df.loc[(df.install_date >= '2022-05-01') & (df.install_date < '2022-11-20')]
...
# 在这里重写是为了满足目标不再是r7/r1-1，而是r7/r1-3
def createDoc(modPath,trainX,trainY0, trainY1,testX,testY0, testY1,history,docDirname,message):
    ...
    yp = trainYP.reshape(-1) + 3*(trainY1.reshape(-1))
    pd.DataFrame(data={
        'yp':list(trainYP.reshape(-1)),
        'y1':list(trainY1.reshape(-1)),
        'true':list(yt),
        'pred':list(yp),
    ...
    yt = testY0.reshape(-1)
    yp = testYP.reshape(-1) + 3*(testY1.reshape(-1))
    pd.DataFrame(data={
        'yp':list(testYP.reshape(-1)),
        'y1':list(testY1.reshape(-1)),
        'true':list(yt),
        'pred':list(yp),
        'mape':list(np.abs((yp - yt) / yt)*100)
...
if __name__ == '__main__':
    if __debug__:
        ...
        # dataStep0()
        dataStep1()
        df = pd.read_csv(getFilename('totalData1'))
        df4 = dataStep3(df)
        df4.to_csv(getFilename('totalData2'))

    df4 = pd.read_csv(getFilename('totalData2'))
    train(df4,'cv deta 01 04')
