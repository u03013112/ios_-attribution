...
def dataStep0(sinceTimeStr,unitlTimeStr):
    ...
    df.to_csv(getFilename('totalData1_3_%s_%s'%(sinceTimeStr,unitlTimeStr)))
# 从本地文件取数据，跑过步骤0的可以直接从这里开始，更快速
def dataStep1(sinceTimeStr,unitlTimeStr):
    df = pd.read_csv(getFilename('totalData1_3_%s_%s'%(sinceTimeStr,unitlTimeStr)))
    return df
...
def dataStep02(sinceTimeStr,unitlTimeStr):
    ...
    print(sql)
    df = execSql(sql)
    df.to_csv(getFilename('totalData2_3_%s_%s'%(sinceTimeStr,unitlTimeStr)))

# 将两部分数据进行合并
def dataStep12(sinceTimeStr,unitlTimeStr):
    df = pd.read_csv(getFilename('totalData1_3_%s_%s'%(sinceTimeStr,unitlTimeStr)))
    df2 = pd.read_csv(getFilename('totalData2_3_%s_%s'%(sinceTimeStr,unitlTimeStr)))
    ...
    df2.to_csv(getFilename('totalData3_3_%s_%s'%(sinceTimeStr,unitlTimeStr)))
    return df2
...
if __name__ == '__main__':
    if __debug__:
        ...
        # dataStep0('20220501','20221215')
        # dataStep02('20220501','20221215')
        df = dataStep12('20220501','20221215')
        df4 = dataStep3(df)
        df4.to_csv(getFilename('totalData4_3_20220501_20221215'))

    df4 = pd.read_csv(getFilename('totalData4_3_20220501_20221215'))
    train(df4,'p3 func2 test')