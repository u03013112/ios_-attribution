# cv改进测试
# 尝试将额外的属性放进cv，希望放入的属性有在线时间、最后支付时间、最大支付金额
# 暂时先将新属性放到首日cv一起
# 最好还是先用纯粹的收日收入先训练一次，因为数据源不一致
...
def dataStep1():
    ...
    df = df.groupby(['install_date','r1max_cv'],as_index=False).agg({'r1usd':'sum','r7usd':'sum','count':'sum'})
    
    df = df.rename(columns={'r1usd':'sumr1usd'})
    df = df.rename(columns={'r7usd':'sumr7usd'})
    df = df.rename(columns={'r1max_cv':'cv'})
...
if __name__ == '__main__':
    if __debug__:
        ...
        df4 = dataStep3(df)
        df4.to_csv(getFilename('totalData2_pmax'))

    df4 = pd.read_csv(getFilename('totalData2_pmax'))
    train(df4,'cv test pmax')
