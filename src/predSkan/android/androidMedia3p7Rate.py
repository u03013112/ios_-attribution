...
##@resource_reference{"androidMedia3Mod20230118.zip"}
...
def predict(dayStr):
    ...
    with zipfile.ZipFile('androidMedia3Mod20230118.zip') as myzip:
...
def createTable():
    ...
    table = o.create_table('topwar_android_media3p7_r7pr3', schema, if_not_exists=True)
    return table
...
def createTable2():
    ...
    schema = Schema(columns=columns, partitions=partitions)
    table = o.create_table('topwar_android_media3p7', schema, if_not_exists=True)
    return table


# import pyarrow as pa
def writeTable(df,dayStr):
    t = o.get_table('topwar_android_media3p7_r7pr3')
...
def writeTable2(df,dayStr):
    t = o.get_table('topwar_android_media3p7')
    t.delete_partition('install_date=%s'%(dayStr), if_exists=True)
    with t.open_writer(partition='install_date=%s'%(dayStr), create_partition=True, arrow=True) as writer:
        writer.write(df)
