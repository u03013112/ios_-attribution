...
##@resource_reference{"androidTotalModRt20230206.zip"}
...
def predict(dayStr):
    name = 'androidTotal'
    with zipfile.ZipFile('androidTotalModRt20230206.zip') as myzip:
...
def createTable():
    ...
    table = o.create_table('topwar_android_total1p7_r7pr1', schema, if_not_exists=True)
    return table
...
def createTable2():
    ...
    schema = Schema(columns=columns, partitions=partitions)
    table = o.create_table('topwar_android_total1p7', schema, if_not_exists=True)
    return table


# import pyarrow as pa
def writeTable(df,dayStr):
    t = o.get_table('topwar_android_total1p7_r7pr1')
...
def writeTable2(df,dayStr):
    t = o.get_table('topwar_android_total1p7')
    t.delete_partition('install_date=%s'%(dayStr), if_exists=True)
    with t.open_writer(partition='install_date=%s'%(dayStr), create_partition=True, arrow=True) as writer:
        writer.write(df)
