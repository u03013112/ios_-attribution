# 继续处理FunPlus02Ssot_redownload的结果
...
def getDataFromMC():
    sql = '''
        ...
        FROM
            topwar_ios_funplus02_ssot_redownload_raw
        where
            day > '20230401'
        GROUP BY
            appsflyer_id,
...
def main():
    df = getDataFromMC()
    df.to_csv(getFilename('funplus02tSsot1redownload'), index=False)
    ...
    df1 = pd.read_csv(getFilename('funplus02tSsot1redownload'))
    ...
    merge_df.to_csv(getFilename('funplus02tSsot4redownload'), index=False)

def getSql1():
    sql = '''
        select * from topwar_ios_funplus02_ssot_redownload_raw where day > 0;
    '''
    print(sql)
    df = execSql(sql)
    return df
...
def rollAndDraw():
    df = pd.read_csv(getFilename('funplus02tSsot4redownload'))
    ...
    for media in df['media'].unique():
        ...
        df1 = df1.reset_index()
        df1.to_csv(getFilename('funplus02tSsot5_%s_redownload' % media), index=False)
    ...
    # 保存图形
    plt.savefig(getFilename('funplus02tSsot6redownload', ext='jpg'))
...
def ewmAndDraw():
    ...
    # 保存图形
    plt.savefig(getFilename('funplus02tSsot6EwmRedownload', ext='jpg'))
