# 与iOSCheck3.py的区别是，这里主要针对4月数据进行计算
...
def getSKANDataFromMC():
    sql = '''
        ...
        WHERE
            day BETWEEN '20230401' AND '20230505'
...
def getAfDataFromMC():
    sql = '''
        ...
        WHERE
            app_id = 'id1479198816'
            AND zone = 0
            AND day BETWEEN '20230401' AND '20230505'
            AND to_date(install_time, "yyyy-mm-dd hh:mi:ss") BETWEEN to_date('20230401', 'yyyyMMdd') AND to_date('20230501', 'yyyyMMdd')
        GROUP BY
            appsflyer_id,
            install_timestamp,
            install_date,
...
def debug():
    df = pd.read_csv('/src/data/zk/skan5.csv')
    # 按照条件汇总，并计算汇总条目
    df['user count'] = 1
