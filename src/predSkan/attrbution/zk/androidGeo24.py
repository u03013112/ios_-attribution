...
def dataStep1():
    ...
    df.to_csv(getFilename('androidFpMergeDataStep1g2_24'), index=False)
    print('dataStep1 done')    

def dataStep2():
    df = pd.read_csv(getFilename('androidFpMergeDataStep1g2_24'))
    ...
    df.to_csv(getFilename('androidFpMergeDataStep2g2_24'), index=False)
...
# 制作一个模拟的SKAN报告
def makeSKAN():
    df = pd.read_csv(getFilename('androidFpMergeDataStep2g2_24'))
...
# 制作待归因用户Df
def makeUserDf():
    userDf = pd.read_csv(getFilename('androidFpMergeDataStep2g2_24'))
...
def meanAttribution(userDf, skanDf):
    ...
    # userDf.to_csv(getFilename('attribution1ReStep48hoursGeo_24'), index=False)
    # userDf.to_parquet(getFilename('attribution1ReStep48hoursGeo_24','parquet'), index=False)
    return userDf
...
def checkRet(retDf):
    ...
    rawDf.to_csv(getFilename('attribution1RetCheck_24'), index=False)
    # 计算整体的MAPE和R2
    ...
    df = pd.read_csv(getFilename('attribution1RetCheck_24'))
...
def debug():
    df = pd.read_csv(getFilename('attribution1RetCheck_24'))
    # df 列 install_date 是类似 2023-01-01 这样的字符串
    # 将 install_date 只保留到月份，即 2023-01
    # 然后按照月份、media分组，计算 MAPE 的平均值
    df['install_date'] = df['install_date'].str.slice(0, 7)
    df = df.groupby(['install_date', 'media']).agg({'MAPE': 'mean'}).reset_index()
    print(df)


def debug2():
    df = pd.read_csv(getFilename('attribution1RetCheck_24'))
    df = df.groupby('media').agg({'MAPE': 'mean'}).reset_index()
    print(df)
...
def fast():
    ...
    userDf = pd.read_csv(getFilename('userAOS6G_24'))
    skanDf = pd.read_csv(getFilename('skanAOS6G_24'))
    
    # 1677628800 是 UTC: Mar. 01, 2023 00:00:00
    skanDf = skanDf.loc[
        (skanDf['media'].isin([
            'googleadwords_int',
            'Facebook Ads'
        ])) &
        (skanDf['min_valid_install_timestamp'] <= 1677628800 ) 
    ]

    userDf = meanAttribution(userDf, skanDf)
    userDf.to_csv(getFilename('attribution1ReStep48hoursGeo_24_fast'), index=False)
    # userDf = pd.read_csv(getFilename('attribution1ReStep48hoursGeo_24_fast'))
    userDf = meanAttributionResult(userDf)
    userDf.to_csv(getFilename('attribution1Ret48Geo_24_fast'), index=False)
    userDf = pd.read_csv(getFilename('attribution1Ret48Geo_24_fast'))
    checkRet(userDf)
...
if __name__ == '__main__':
    ...
    # skanDf = skanAddGeo()
    # skanDf.to_csv(getFilename('skanAOS6G_24'), index=False)

    # userDf = makeUserDf()
    # print('user data len:',len(userDf))
    # userDf = userInstallDate2Min(userDf,N = 600)
    # userDf = userGroupby(userDf)
    # userDf.to_csv(getFilename('userAOS6G_24'),index=False)
    ...
    # userDf = meanAttribution(userDf, skanDf)
    # userDf.to_csv(getFilename('attribution1ReStep48hoursGeo_24'), index=False)
    # # userDf = pd.read_csv(getFilename('attribution1ReStep48hoursGeo_24'))
    # userDf = meanAttributionResult(userDf)
    # userDf.to_csv(getFilename('attribution1Ret48Geo_24'), index=False)
    # userDf = pd.read_csv(getFilename('attribution1Ret48Geo_24'))
    # checkRet(userDf)
