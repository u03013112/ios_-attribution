# 与iOSCheck2.py的区别在于，尝试使用AF的归因结论
# 主要修改如下：
# 在获得AF数据时，添加字段media，用于记录AF归因结论
# 然后将AF数据中有归因结论的数据，按照安装日期与归因结论进行分组，计算r7usd
# 归因这边，将SKAN中CV>=32的数据舍弃，由于这部分是SSOT标记的重复归因
# 用户数据这边也只保留media字段为空的用户
# 进行归因后获得按照安装日期与归因结论进行分组，计算r7usd
# 然后将两部分数据进行合并，得到最终的r7usd
...
def meanAttributionResult(userDf, mediaList=mediaList):
    ...
    # Save to CSV
    userDf.to_csv(getFilename('attribution1Ret0504'), index=False)
    return userDf
...
def main():
    ...
    skanDf = skanDf.loc[
        (skanDf['postback_timestamp'] >= '2023-03-05') &
...
import numpy as np
def debug3():
    df = pd.read_csv('/src/data/zk/merged_result.csv')
    # 去掉Date列是‘2023/3/10’的行
    df = df.loc[df['Date'] != '2023/3/10']

    # df中列‘SKAN ROI（Real）’和‘ROAS(D2)’和‘概率归因D7 ROI’格式都是类似‘11.83%’的str，先转成float
    df['SKAN ROI（Real）'] = df['SKAN ROI（Real）'].str.replace('%', '')
    df['SKAN ROI（Real）'] = pd.to_numeric(df['SKAN ROI（Real）'], errors='coerce')/100
    df['ROAS(D2)'] = df['ROAS(D2)'].str.replace('%', '')
    df['ROAS(D2)'] = pd.to_numeric(df['ROAS(D2)'], errors='coerce')/100
    # df['ROAS(D7)预测'] = df['ROAS(D7)预测'].str.replace('%', '')
    # df['ROAS(D7)预测'] = pd.to_numeric(df['ROAS(D7)预测'], errors='coerce')/100

    df['r1usd'] = df['Cost']*df['SKAN ROI（Real）']
    df['r2usd'] = df['Cost']*df['ROAS(D2)']
    df['ssot r7usd'] = df['Cost']*df['ROAS(D7)预测']
    df = df.groupby(['media']).agg({
        'r1usd':'sum',
        'r2usd':'sum',
        'ssot r7usd':'sum',
        'r7usd':'sum'
    })
    # 分媒体(media列）计算，df['ssot r7usd']/df['r1usd']
    # 分媒体(media列）计算，df['ssot r7usd']/df['r2usd']
    # 分媒体(media列）计算，df['r7usd']/df['r1usd']
    # 分媒体(media列）计算，df['r7usd']/df['r2usd']
    df['ssot r7usd/r1usd'] = df['ssot r7usd']/df['r1usd']
    df['ssot r7usd/r2usd'] = df['ssot r7usd']/df['r2usd']
    df['r7usd/r1usd'] = df['r7usd']/df['r1usd']
    df['r7usd/r2usd'] = df['r7usd']/df['r2usd']

    df = df [['ssot r7usd/r1usd','ssot r7usd/r2usd','r7usd/r1usd','r7usd/r2usd']]
    print(df)
    df.to_csv(getFilename('debug3'))

def debug4():
    # skan报告3月4日~3月22日期间，分媒体查看cv分布
    # 统计4个主要媒体的CV分布
    # 其中分为3档，1~10，11~20，21~31
    df = pd.read_csv('/src/data/zk/skan.csv')
    # 排除2023-03-10的数据
    df = df.loc[df['install_date'] != '2023-03-10']
    # cv > 31的数据，cv列 -= 32
    df.loc[df['cv'] > 31, 'cv'] -= 32

    # 按照添加cv group列，按照cv，将cv分为3档，1~10，11~20，21~31
    df['cv group'] = np.where(df['cv'] <= 10, '01~10', np.where(df['cv'] <= 20, '11~20', '21~31'))
    # df['cv group'] = (df['cv'] // 11).astype(int)

    # 按媒体，cv汇总，统计行数
    df = df.groupby(['media','cv group']).agg({'event_uuid':'count'})
    # 添加一列，计算每个媒体中的cv group的占比
    df['cv group rate'] = df.groupby(level=0).apply(lambda x: x / float(x.sum()))
    # 将cv group rate列转换为百分比
    df['cv group rate'] = df['cv group rate'].apply(lambda x: format(x, '.2%'))
    
    print(df)
    df.to_csv(getFilename('debug4'))
    
def debug5():
    # 尝试按照用户的首日付费金额（cv）分档，分档为1~10，11~20，21~31
    # 计算r7usd/r1usd
    userDf = pd.read_csv('/src/data/zk/user.csv')
    userDf.loc[:,'install_date'] = pd.to_datetime(userDf['install_timestamp']).dt.date.astype(str)
    # install_date排除2023-03-10的数据
    userDf = userDf.loc[userDf['install_date'] != '2023-03-10']

    # r1usd列空值填充0
    userDf['r1usd'] = userDf['r1usd'].fillna(0)
    # r7usd列空值填充0
    userDf['r7usd'] = userDf['r7usd'].fillna(0)
    # cv转成int
    userDf['cv'] = userDf['cv'].astype(int)
    # 排除cv == 0 的数据
    userDf = userDf.loc[userDf['cv'] != 0]
    userDf['cv group'] = np.where(userDf['cv'] <= 10, '01~10', np.where(userDf['cv'] <= 20, '11~20', '21~31'))
    
    df = userDf.groupby(['cv group']).agg({'appsflyer_id':'count','r1usd':'sum','r7usd':'sum'})
    # df = userDf.groupby(['cv']).agg({'appsflyer_id':'count','r1usd':'sum','r7usd':'sum'})

    # 计算r7usd/r1usd
    df['r7usd/r1usd'] = df['r7usd']/df['r1usd']

    # 添加一列，计算每个cv group中appsflyer_id的占比
    total_appsflyer_id = df['appsflyer_id'].sum()
    df['cv group rate'] = df['appsflyer_id'] / total_appsflyer_id

    # 将cv group rate列转换为百分比
    df['cv group rate'] = df['cv group rate'].apply(lambda x: format(x, '.2%'))

    print(df)
    df.to_csv(getFilename('debug5'))


def debug6():
    # df = getAndroidDataFromMC()
    # df.to_csv('/src/data/zk/afAndroid.csv')
    df = pd.read_csv('/src/data/zk/afAndroid.csv')
    # Replace NaN values with empty strings in the 'media' column
    df['media'] = df['media'].fillna('')
    # 过滤掉已经有归因的部分
    df = df.loc[df['media'] == ''].copy()
    ...
    print(df)
    df.to_csv(getFilename('debug6'))


from sklearn.metrics import r2_score
def ret():
    afDf = pd.read_csv('/src/data/zk/afR7Usd.csv')
    ...
    resultDf = pd.read_csv('/src/data/zk/attribution1Ret0504.csv')
    # print('resultDf',resultDf['media'].unique())
    resultDf = resultDf.merge(afDf, on=['media','install_date'], how='left')
    # print('merge:',resultDf['media'].unique())
    # print(resultDf.loc[resultDf['media'] == 'Facebook Ads'])

    resultDf.rename(columns={'r7usd': 'af r7usd'}, inplace=True)
    resultDf['af r7usd'] = resultDf['af r7usd'].fillna(0)

    resultDf['r7usd'] = resultDf['af r7usd'] + resultDf['r7usdp']    
...
# 将AF数据中有归因结论的数据，按照安装日期与归因结论进行分组，计算r7usd
def getAfR7Usd():
    df = pd.read_csv('/src/data/zk/af.csv')
    df = df.groupby(['install_date', 'media']).agg({'r7usd': 'sum'}).reset_index()
    df.to_csv('/src/data/zk/afR7Usd.csv', index=False)
