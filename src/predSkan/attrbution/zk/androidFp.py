...
#     # 需要记录日志的appsflyer_id列表
#     target_appsflyer_ids = [
#         '1672557684460-2251199065155050476',
#         '1674781062979-8243551550556056010',
#         '1679770383622-1215911809165406356',
#     ]
#     total_rows = len(skanDf)

#     # 打开文件以写入操作日志
#     with open(getFilename('operation_log'), 'w') as operation_log_file:
#         # 写入CSV文件头
#         operation_log_file.write('skanDf_index,appsflyer_id,column,increment\n')
...
#             if num_matching_rows > 0:
#                 # 更新userDf并记录操作日志
#                 increment = 1 / num_matching_rows
#                 userDf.loc[condition, media + ' count'] += increment

#                 for _, matching_row in matching_rows.iterrows():
#                     # 如果appsflyer_id在目标列表中，则将操作日志写入文件
#                     if matching_row['appsflyer_id'] in target_appsflyer_ids:
#                         operation_log_file.write(f"{index},{matching_row['appsflyer_id']},{media + ' count'},{increment}\n")
...
# 针对attribution1的结果，检查是否有行的所有的media count列的和大于1
# 并将media count列的和大于1的行打印出来，以便人工检查
# 可能行数太多，直接保存为csv，以便人工检查
def invalidRowsCheck(userDf):
    # 获取所有的media count列
    media_count_columns = [col for col in userDf.columns if ' count' in col]

    # 计算所有媒体计数列的和
    media_counts_sum = userDf[media_count_columns].sum(axis=1)

    # 检查这些和是否大于1
    invalid_rows = media_counts_sum > 1

    # 提取无效行并保存到CSV文件中
    invalid_userDf = userDf[invalid_rows]

    # 计算每行的媒体计数总和并添加到新列
    invalid_userDf.loc[:,'media_count_sum'] = media_counts_sum[invalid_rows]

    # 按照media_count_sum降序排列
    invalid_userDf = invalid_userDf.sort_values(by='media_count_sum', ascending=False)

    invalid_userDf.to_csv(getFilename('invalid_rows'), index=False)

    print(f"Invalid rows have been saved to 'invalid_rows.csv'")
...
def debug():
    ...
    # logDf columns: skanDf_index,appsflyer_id,column,increment
    logDf = pd.read_csv(getFilename('operation_log'))
    # userDf columns: appsflyer_id,install_timestamp,r1usd,r7usd,cv
    userDf = pd.read_csv(getFilename('user'))
    # skanDf columns: postback_timestamp,media,cv,min_valid_install_timestamp,max_valid_install_timestamp,postback_date,min_valid_install_date,max_valid_install_date
    skanDf = pd.read_csv(getFilename('skan'))
    # 打印skanDf的所有列名，和类型
    # print(skanDf.dtypes)

    # 
    afId = target_appsflyer_ids[0]

    # 在logDf中找到指定afId对应的skanDf_index
    skanDf_indices = logDf.loc[logDf['appsflyer_id'] == afId, 'skanDf_index'].unique()

    # 打印对应的skanDf中的行
    for skanDf_index in skanDf_indices:
        tmpDf = skanDf.loc[skanDf.index == skanDf_index]
        tmpDf = tmpDf[['media','cv']]
        print(tmpDf)

    # 打印对应的userDf中的行
    print(userDf.loc[userDf['appsflyer_id'] == afId])

# 查询所有可以匹配到指定af id的skan行，并打印出来
def debug1(afId):
    # userDf columns: appsflyer_id,install_timestamp,r1usd,r7usd,cv
    userDf = pd.read_csv(getFilename('user'))
    # skanDf columns: postback_timestamp,media,cv,min_valid_install_timestamp,max_valid_install_timestamp,postback_date,min_valid_install_date,max_valid_install_date
    ...
    user = userDf.loc[userDf['appsflyer_id'] == afId]
    # user 应该只有一行，如果不是报错，退出
    if len(user) != 1:
        raise ValueError(f"Invalid number of rows for appsflyer_id {afId}: {len(user)}")

    # 按照user去找到所有可以匹配的skanDf的行，要求userDf.cv == skanDf.cv，并且 skanDf.max_valid_install_timestamp >= userDf.install_timestamp >= skanDf.min_valid_install_timestamp 的行。
    matching_rows = skanDf.loc[
        (skanDf['cv'] == user.iloc[0]['cv']) &
        (skanDf['max_valid_install_timestamp'] >= user.iloc[0]['install_timestamp']) &
        (skanDf['min_valid_install_timestamp'] <= user.iloc[0]['install_timestamp'])
    ]
