...
def main24(fast = False,onlyCheck = False):
    if onlyCheck == False:
        if fast == False:
            ...
            skanDf = pd.read_csv(getFilename('skanAOSCampaignG24'))
        ...
        meanAttributionFastv2Filename = getFilename(f'meanAttribution24_{installTimeStart}_{installTimeEnd}')
        userDf.to_csv(meanAttributionFastv2Filename, index=False)
        userDf = pd.read_csv(meanAttributionFastv2Filename)
        userDf = meanAttributionResult(userDf,mediaList)
        meanAttributionResultFilename = getFilename(f'meanAttributionResult24_{installTimeStart}_{installTimeEnd}')
        userDf.to_csv(meanAttributionResultFilename, index=False)

    meanAttributionResultFilename = getFilename(f'meanAttributionResult24_{installTimeStart}_{installTimeEnd}')
    userDf = pd.read_csv(meanAttributionResultFilename)
    df = checkRet(userDf)
...
def debug():
    ...
    for day in dayList:
        ...
        filename = getFilename(f'attMedia24_groupbyMedia_Mape_{installTimeStart}_{installTimeEnd}')
    ...
    df.to_csv(getFilename('attMedia24_groupbyMedia_Mape'),index=False)
