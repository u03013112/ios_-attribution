...
def loadData():
    # 加载数据
    df = pd.read_csv(getFilename('androidFp07Vn'))
...
def dataStep1():
    ...
    df.to_csv(getFilename('androidFpMergeDataStep1g2Vn'), index=False)
    print('dataStep1 done')    

def dataStep2():
    df = pd.read_csv(getFilename('androidFpMergeDataStep1g2Vn'))
    ...
    df.to_csv(getFilename('androidFpMergeDataStep2g2Vn'), index=False)
...
# 制作一个模拟的SKAN报告
def makeSKAN():
    df = pd.read_csv(getFilename('androidFpMergeDataStep2g2Vn'))
...
# 制作待归因用户Df
def makeUserDf():
    userDf = pd.read_csv(getFilename('androidFpMergeDataStep2g2Vn'))
...
def draw():
    ...
    for media in mediaList:
        ...
        plt.savefig(f'/src/data/zk2/attVN_{media}.jpg', bbox_inches='tight')
        plt.close()

def debug():
    df = pd.read_csv(getFilename('attribution1RetCheck'))
    # install_date 是类似 2021-01-01 的字符串
    # 先按照media，再按照install_date排序，然后计算7日均线的MAPE
    df = df.sort_values(['media', 'install_date'])
    for media in mediaList:
        media_df = df[df['media'] == media].copy(deep=True)
        # media_df['MAPE_7d'] = media_df['MAPE'].rolling(7).mean()
        # 分别对r7usd和r7usdp计算7日均线，然后再计算MAPE
        media_df['r7usd_7d'] = media_df['r7usd'].rolling(7).mean()
        media_df['r7usdp_7d'] = media_df['r7usdp'].rolling(7).mean()
        media_df['MAPE_7d'] = abs(media_df['r7usd_7d'] - media_df['r7usdp_7d']) / media_df['r7usd_7d']
        print('media:',media)
        print('7日均线MAPE:',media_df['MAPE_7d'].mean())
        
        # 计算总金额的MAPE
        print('总金额MAPE:',abs(media_df['r7usd_7d'].mean() - media_df['r7usdp_7d'].mean()) / media_df['r7usd_7d'].mean())

        media_df.to_csv(f'/src/data/zk2/attVN_{media}.csv', index=False)
...
if __name__ == '__main__':
    ...
    # skanDf.to_csv(getFilename('skanAOS6G48Vn'), index=False)

    # userDf = makeUserDf()
    # print('user data len:',len(userDf))
    # userDf.to_csv(getFilename('userAOS6Vn'),index=False)
    # # userDf = pd.read_csv(getFilename('userAOS6Vn'))
    # userDf = userInstallDate2Min(userDf,N = 600)
    # userDf = userGroupby(userDf)
    # userDf.to_csv(getFilename('userAOS6G48Vn'),index=False)
    # print('user data group len:',len(userDf))

    # userDf = pd.read_csv(getFilename('userAOS6G48Vn'))
    # skanDf = pd.read_csv(getFilename('skanAOS6G48Vn'))
    
    # userDf = meanAttribution(userDf, skanDf)
    # userDf.to_csv(getFilename('attribution1ReStep48hoursVn'), index=False)
    # # userDf = pd.read_csv(getFilename('attribution1ReStep48hoursVn'))
    # userDf = meanAttributionResult(userDf)
    # userDf.to_csv(getFilename('attribution1Ret48Vn'), index=False)
    # # userDf = pd.read_csv(getFilename('attribution1Ret48Vn'))
    # checkRet(userDf)
