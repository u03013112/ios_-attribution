# 在FunPlus02.py的基础上进行改进
# 将模糊归因的结论排除在外
# 只针对没有归因的用户进行归因，这个结论应该更加贴近SSOT
...
def createTable():
    if 'o' in globals():
        ...
        table = o.create_table('topwar_ios_funplus02_ssot_raw', schema, if_not_exists=True)
        return table
    else:
...
def writeTable(df,dayStr):
    ...
    print(df.head(5))
    if 'o' in globals():
        t = o.get_table('topwar_ios_funplus02_ssot_raw')
