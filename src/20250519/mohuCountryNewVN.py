...
def getRevenueData(startDayStr, endDayStr):
    filename = f'/src/data/vn_mohu_country_revenue_{startDayStr}_{endDayStr}.csv'
...
def bayesianCountryDataPrepare(startDayStr, endDayStr):
    mediaList = [
        'Facebook Ads','applovin_int','bytedanceglobal_int'
    ]

    revenueDf = getRevenueData(startDayStr, endDayStr)
    # 越南包忽略国家
    revenueDf['country'] = 'VN'
    ...
    print('media:',revenueDf['mediasource'].unique())
    ...
    prepareDf.to_csv(f'/src/data/vn_lw_mohu_bayesian_country_{startDayStr}_{endDayStr}.csv', index=False)
    return prepareDf
...
def main(dayStr = None):
    ...
    prepareWeekDf.to_csv(f'/src/data/vn_countryMain_{startDayStr}_{endDayStr}.csv', index=False)
    ...
    for country in countryList:
        ...
        for organicConfigRevenue in organicRevenueConfigList:
            ...
            with basic_model as model:
                ...
                detailDf.to_csv(f'/src/data/vn_detailByDay_{country}_{organicConfigRevenue["mu"]}_{startDayStr}_{endDayStr}.csv', index=False)
                
                detailDf = detailDf[['install_week','actual_revenue','predicted_revenue']]
                detailDf = detailDf.rename(columns={
                    'predicted_revenue': f'predicted_revenue_{organicConfigRevenue["mu"]}',
                })
                if detailMergeDf.empty:
                    detailMergeDf = detailDf
                else:
                    detailMergeDf = pd.merge(detailMergeDf, detailDf, on=['install_week','actual_revenue'])

                resultDf = pd.concat([resultDf, retDf], ignore_index=True)

        print(resultDf)
        resultDf.to_csv(f'/src/data/vn_mohuByDay_result_{country}_{startDayStr}_{endDayStr}.csv', index=False)
        detailMergeDf.to_csv(f'/src/data/vn_detailByDay_{country}_{startDayStr}_{endDayStr}.csv', index=False)
        ...
        print(resultDf)
        resultDf.to_csv(f'/src/data/vn_resultByDay_{country}_{startDayStr}_{endDayStr}_kpi.csv', index=False)
    ...
    resultDfTotal = resultDfTotal[['country','organic_revenue_mu','applovin_x','facebook_x','tiktok_x','mape','organic_ratio','7d_kpi','applovin_kpi','facebook_kpi','tiktok_kpi']]
    resultDfTotal.to_csv(f'/src/data/vn_mohuByDayCountry_result_total_{startDayStr}_{endDayStr}.csv', index=False)
