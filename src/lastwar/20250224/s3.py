...
def check_directory(s3_client, bucket_name, directory_name):
    """
    检查 S3 中的目录是否存在
...
def create_directory(s3_client, bucket_name, directory_name):
    """
...
def upload_file_to_s3(s3_client, file_name, bucket_name, directory_name, object_name=None):
    """
    将文件上传到指定的 S3 目录
...
if __name__ == "__main__":
    ...
    # 检查目录是否存在，如果不存在则创建
    if not check_directory(s3_client, bucket_name, directory_name):
        print(f"目录 {directory_name} 不存在，正在创建...")
        create_directory(s3_client, bucket_name, directory_name)
    else:
        print(f"目录 {directory_name} 已存在")

    # # 上传文件到指定目录
    # upload_file_to_s3(s3_client, file_name, bucket_name, directory_name)