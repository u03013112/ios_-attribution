# 逻辑复制predictRevenue3_36_sum.py
# 只是作出以下修改
# 1、Prophet模型训练时添加参数changepoint_prior_scale、seasonality_prior_scale
# 暂时先使用changepoint_prior_scale = 0.1、seasonality_prior_scale = 0.5
# 2、之前是使用最后56天作为测试集，然后再将所有数据均作为训练集
# 改为使用最后56天作为测试集，但是只使用测试集前的56天作为训练集
# 同样的，最终的预测结果也只使用最后56天的数据作为训练集
...
def prophet1FloorL(today = None,future_periods=60):
    ...
    # trainStartDay = testStartDay - pd.Timedelta(days=7*8)
    # trainStartDayStr = trainStartDay.strftime('%Y-%m-%d')
    trainStartDayStr = '2024-10-16'
    ...
    train_df = server_df[['ds', 'y']][(server_df['ds'] >= trainStartDayStr) & (server_df['ds'] < start_date)]
