...
def totalAndPlatformCountry():
    ...
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    ax.plot(JP_IOSDf['install_day'], JP_IOSDf['KPI'], label='KPI', color='blue')
    ax.plot(JP_IOSDf['install_day'], JP_IOSDf['sum_7roi'], label='sum 7roi', color='green')
    ax.axvline(x=full7dayEndDate, color='orange', linestyle='--', label='full7dayEndDate')
    ax.set_xlabel('Install Day')
    ax.set_ylabel('ROI')
    ax.set_title('IOS JP 7ROI')
    ax.legend()
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('/src/data/th_lichengbei_JP_IOS_ROI.png')
    plt.close()
    # 双y轴，一个cost（每日花费），一个sum_cost_ok（累计达标花费）
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    ax.plot(JP_IOSDf['install_day'], JP_IOSDf['cost'], label='daily cost', color='blue')
    ax.legend(loc='upper left')
    ax.set_ylabel('daily cost')
    ax2 = ax.twinx()
    ax2.fill_between(JP_IOSDf['install_day'], JP_IOSDf['sum_cost_ok'], color='green', alpha=0.5, label='sum cost ok')
    # ax2.plot(JP_IOSDf['install_day'], JP_IOSDf['sum_cost_ok'], label='sum cost ok', color='green')
    ax2.legend(loc='upper right')
    ax2.set_ylabel('sum cost ok')
    ax.axvline(x=full7dayEndDate, color='orange', linestyle='--', label='full7dayEndDate')
    ax.set_xlabel('Install Day')
    ax.set_title('IOS JP daily cost and sum cost ok')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('/src/data/th_lichengbei_JP_IOS_cost.png')
    plt.close()
    ...
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    ax.plot(NOJP_AOSDf['install_day'], NOJP_AOSDf['KPI'], label='KPI', color='blue')
    ax.plot(NOJP_AOSDf['install_day'], NOJP_AOSDf['sum_7roi'], label='sum 7roi', color='green')
    ax.axvline(x=full7dayEndDate, color='orange', linestyle='--', label='full7dayEndDate')
    ax.set_xlabel('Install Day')
    ax.set_ylabel('ROI')
    ax.set_title('AOS NOJP 7ROI')
    ax.legend()
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('/src/data/th_lichengbei_NOJP_AOS_ROI.png')
    plt.close()
    # 双y轴，一个cost（每日花费），一个sum_cost_ok（累计达标花费）
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    ax.plot(NOJP_AOSDf['install_day'], NOJP_AOSDf['cost'], label='daily cost', color='blue')
    ax.legend(loc='upper left')
    ax.set_ylabel('daily cost')
    ax2 = ax.twinx()
    ax2.fill_between(NOJP_AOSDf['install_day'], NOJP_AOSDf['sum_cost_ok'], color='green', alpha=0.5, label='sum cost ok')
    # ax2.plot(NOJP_AOSDf['install_day'], NOJP_AOSDf['sum_cost_ok'], label='sum cost ok', color='green')
    ax2.legend(loc='upper right')
    ax2.set_ylabel('sum cost ok')
    ax.axvline(x=full7dayEndDate, color='orange', linestyle='--', label='full7dayEndDate')
    ax.set_xlabel('Install Day')
    ax.set_title('AOS NOJP daily cost and sum cost ok')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('/src/data/th_lichengbei_NOJP_AOS_cost.png')
    plt.close()
    ...
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    ax.plot(NOJP_IOSDf['install_day'], NOJP_IOSDf['KPI'], label='KPI', color='blue')
    ax.plot(NOJP_IOSDf['install_day'], NOJP_IOSDf['sum_7roi'], label='sum 7roi', color='green')
    ax.axvline(x=full7dayEndDate, color='orange', linestyle='--', label='full7dayEndDate')
    ax.set_xlabel('Install Day')
    ax.set_ylabel('ROI')
    ax.set_title('IOS NOJP 7ROI')
    ax.legend()
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('/src/data/th_lichengbei_NOJP_IOS_ROI.png')
    plt.close()
    # 双y轴，一个cost（每日花费），一个sum_cost_ok（累计达标花费）
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    ax.plot(NOJP_IOSDf['install_day'], NOJP_IOSDf['cost'], label='daily cost', color='blue')
    ax.legend(loc='upper left')
    ax.set_ylabel('daily cost')
    ax2 = ax.twinx()
    ax2.fill_between(NOJP_IOSDf['install_day'], NOJP_IOSDf['sum_cost_ok'], color='green', alpha=0.5, label='sum cost ok')
    # ax2.plot(NOJP_IOSDf['install_day'], NOJP_IOSDf['sum_cost_ok'], label='sum cost ok', color='green')
    ax2.legend(loc='upper right')
    ax2.set_ylabel('sum cost ok')
    ax.axvline(x=full7dayEndDate, color='orange', linestyle='--', label='full7dayEndDate')
    ax.set_xlabel('Install Day')
    ax.set_title('IOS NOJP daily cost and sum cost ok')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('/src/data/th_lichengbei_NOJP_IOS_cost.png')
    plt.close()
