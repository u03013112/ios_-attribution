# 用付费用户数预测收入金额，按周进行预测，计算每周的MAPE
# 训练使用真实付费用户数，预测使用预测付费用户数
import os
import pandas as pd
import numpy as np
...
def train(train_df):
    inputCols = ['ds','ad_spend','pud1', 'is_weekend']
    outputCols = ['y']
...
def main(group_by_media=False, group_by_country=False):
    ...
    for media in mediaList:
        for country in countryList:
            ...
            while current_date <= end_date:
                ...
                future_df = df[df['ds'].isin(future_dates)][['ds','ad_spend','pud1 predicted', 'is_weekend', 'y']].copy()
                future_df.rename(columns={'pud1 predicted': 'pud1'}, inplace=True)
    ...
    output_filename = f'/src/data/weekly_revenue_{group_name_str}_2.csv'
    detail_output_filename = f'/src/data/weekly_revenue_{group_name_str}_2_detail.csv'
...
def test():
    # 读取所有组合的结果文件
    df_all_all = pd.read_csv('/src/data/weekly_revenue_all_all_2_detail.csv')
    df_all_country = pd.read_csv('/src/data/weekly_revenue_all_country_2_detail.csv')
    df_media_all = pd.read_csv('/src/data/weekly_revenue_media_all_2_detail.csv')
    df_media_country = pd.read_csv('/src/data/weekly_revenue_media_country_2_detail.csv')

    df_all_all_week = pd.read_csv('/src/data/weekly_revenue_all_all_2.csv')
    df_all_country_week = pd.read_csv('/src/data/weekly_revenue_all_country_2.csv')
    df_media_all_week = pd.read_csv('/src/data/weekly_revenue_media_all_2.csv')
    df_media_country_week = pd.read_csv('/src/data/weekly_revenue_media_country_2.csv')
