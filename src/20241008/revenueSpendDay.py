...
def main(group_by_media=False, group_by_country=False):
    ...
    for media in mediaList:
        for country in countryList:
            ...
            while current_date <= end_date:
                ...
                # 定义未来1天的预测集
                future_df = df[df['ds'] == current_date][['ds', 'ad_spend', 'is_weekend', 'y']].copy()
    ...
    # 输出查询表单
    print("Daily Results DataFrame:")
    print(results_df[['ds', 'yhat', 'y', 'mape']])

    # 输出到 CSV 文件
    group_name_str = f"{'media' if group_by_media else 'all'}_{'country' if group_by_country else 'all'}"
    output_filename = f'/src/data/daily_revenue_{group_name_str}.csv'
    results_df.to_csv(output_filename, index=False)

    print(f"Daily Results DataFrame has been saved to {output_filename}")

    # 计算并输出每天收入的MAPE的平均值
    average_mape = results_df['mape'].mean()
    print(f"每天收入的MAPE的平均值: {average_mape:.2f}%")

def test():
    # 读取所有组合的结果文件
    df_all_all = pd.read_csv('/src/data/daily_revenue_all_all.csv')
    df_all_country = pd.read_csv('/src/data/daily_revenue_all_country.csv')
    df_media_all = pd.read_csv('/src/data/daily_revenue_media_all.csv')
    df_media_country = pd.read_csv('/src/data/daily_revenue_media_country.csv')
