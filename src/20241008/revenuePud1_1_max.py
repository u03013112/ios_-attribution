...
select
    ...
    sum(user_revenue_24h) as 24hours_revenue,
    sum(least(revenue_1d, {limit})) as 1day_revenue_capped,
    sum(revenue_1d) as 1day_revenue,
    country,
    mediasource
from (
    select
...
def preprocessData(data,capped_data0, media=None, country=None):
    ...
    # 合并削弱大R版本的数据
    capped_aggregated_data = capped_aggregated_data.rename(columns={'date': 'ds', '1day_revenue_capped': 'y'})
...
def main(N=400,group_by_media=False, group_by_country=False):
    ...
    output_filename = f'/src/data/weekly_revenue_{group_name_str}_1_{N}.csv'
    detail_output_filename = f'/src/data/weekly_revenue_{group_name_str}_1_detail_{N}.csv'
...
def test(N=400):
    # 读取所有组合的结果文件
    df_all_all = pd.read_csv('/src/data/weekly_revenue_all_all_1_detail_{N}.csv')
    df_all_country = pd.read_csv('/src/data/weekly_revenue_all_country_1_detail_{N}.csv')
    df_media_all = pd.read_csv('/src/data/weekly_revenue_media_all_1_detail_{N}.csv')
    df_media_country = pd.read_csv('/src/data/weekly_revenue_media_country_1_detail_{N}.csv')

    df_all_all_week = pd.read_csv('/src/data/weekly_revenue_all_all_1_{N}.csv')
    df_all_country_week = pd.read_csv('/src/data/weekly_revenue_all_country_1_{N}.csv')
    df_media_all_week = pd.read_csv('/src/data/weekly_revenue_media_all_1_{N}.csv')
    df_media_country_week = pd.read_csv('/src/data/weekly_revenue_media_country_1_{N}.csv')
...
def testN(NList):
    ...
    results_dict = {
        ...
        'mape_all_all_week': [],
        'mape_by_media_week': {},
        'mape_by_country_week': {},
        'mape_by_media_country_week': {}
    }

    for N in NList:
        # 读取所有组合的结果文件
        df_all_all_week = pd.read_csv(f'/src/data/weekly_revenue_all_all_1_{N}.csv')
        df_all_country_week = pd.read_csv(f'/src/data/weekly_revenue_all_country_1_{N}.csv')
        df_media_all_week = pd.read_csv(f'/src/data/weekly_revenue_media_all_1_{N}.csv')
        df_media_country_week = pd.read_csv(f'/src/data/weekly_revenue_media_country_1_{N}.csv')

        # 计算并存储所有数据的平均MAPE
        mape_all_all_week = df_all_all_week['mape'].mean() / 100
        results_dict['mape_all_all_week'].append(mape_all_all_week)

        # 计算并存储按媒体分组的平均MAPE
        mape_by_media_week = df_media_all_week.groupby('media')['mape'].mean() / 100
        for media, mape in mape_by_media_week.items():
            if media not in results_dict['mape_by_media_week']:
                results_dict['mape_by_media_week'][media] = []
            results_dict['mape_by_media_week'][media].append(mape)

        # 计算并存储按国家分组的平均MAPE
        mape_by_country_week = df_all_country_week.groupby('country')['mape'].mean() / 100
        for country, mape in mape_by_country_week.items():
            if country not in results_dict['mape_by_country_week']:
                results_dict['mape_by_country_week'][country] = []
            results_dict['mape_by_country_week'][country].append(mape)

        # 计算并存储按媒体和国家分组的平均MAPE
        mape_by_media_country_week = df_media_country_week.groupby(['media', 'country'])['mape'].mean() / 100
        for (media, country), mape in mape_by_media_country_week.items():
            if (media, country) not in results_dict['mape_by_media_country_week']:
                results_dict['mape_by_media_country_week'][(media, country)] = []
            results_dict['mape_by_media_country_week'][(media, country)].append(mape)

    # 将结果转换为DataFrame
    results_df = pd.DataFrame({'N': NList})
    results_df['mape_all_all_week'] = results_dict['mape_all_all_week']

    for media in results_dict['mape_by_media_week']:
        results_df[f'mape_by_media_week_{media}'] = results_dict['mape_by_media_week'][media]

    for country in results_dict['mape_by_country_week']:
        results_df[f'mape_by_country_week_{country}'] = results_dict['mape_by_country_week'][country]

    for (media, country) in results_dict['mape_by_media_country_week']:
        results_df[f'mape_by_media_country_week_{media}_{country}'] = results_dict['mape_by_media_country_week'][(media, country)]

    # 输出到 CSV 文件
    output_filename = '/src/data/mape_comparison.csv'
    results_df.to_csv(output_filename, index=False)

    print(f"MAPE Comparison DataFrame has been saved to {output_filename}")
