...
def main():
    ...
    while current_date <= end_date:
        ...
        if not test_df.empty:
            ...
            test_df = test_df.rename(columns={'yhat': 'predicted_pud1_pct'})

            # 计算MAPE
            test_df['mape'] = np.abs((test_df['y'] - test_df['predicted_pud1_pct']) / (1 + test_df['y'])) * 100
    ...
    print(results_df[['ds', 'predicted_pud1_pct', 'y', 'mape']])

    # 输出到 CSV 文件
    results_df.to_csv('/src/data/pud1_pct_prediction_results.csv', index=False)
