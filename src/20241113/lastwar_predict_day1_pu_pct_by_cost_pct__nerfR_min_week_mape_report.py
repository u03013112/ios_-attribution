...
def createTable():
    if 'o' in globals():
        ...
        table_name = 'lastwar_predict_day1_pu_pct_by_cost_pct__nerfR_min_week_mape_report'
...
select
    ...
    predicted_revenue,
    max_r
from lastwar_predict_day1_pu_pct_by_cost_pct__nerfR_test
where day > 0
...
def getMinWeekMape(installDayStart, installDayEnd, app):
    ...
    # 计算 天MAPE
    dayDf = historical_data.groupby(['install_day', 'media', 'country', 'group_name','max_r']).agg({
        'actual_revenue': 'sum',
        'predicted_revenue': 'sum'
    }).reset_index()

    dayDf['mape_revenue'] = np.abs((dayDf['actual_revenue'] - dayDf['predicted_revenue']) / dayDf['actual_revenue'])
    dayDf2 = dayDf.groupby(['media', 'country', 'group_name','max_r']).agg({
        'mape_revenue': 'mean'
    }).reset_index()

    # 计算 周MAPE
    historical_data['week'] = historical_data['install_day'].dt.strftime('%Y-%W')
    weekDf = historical_data.groupby(['week', 'media', 'country', 'group_name','max_r']).agg({
        'actual_revenue': 'sum',
        'predicted_revenue': 'sum'
    }).reset_index()
    weekDf['mape_revenue'] = np.abs((weekDf['actual_revenue'] - weekDf['predicted_revenue']) / weekDf['actual_revenue'])

    weekDf2 = weekDf.groupby(['media', 'country', 'group_name','max_r']).agg({
        'mape_revenue': 'mean'
    }).reset_index()

    # 找到按照 media 和 country 分组的最小 MAPE 对应的 group_name，以及最小的 MAPE 值
    minMapeDf2 = weekDf2.groupby(['media', 'country','max_r']).agg(
        minMape=('mape_revenue', 'min')
    ).reset_index()

    minMapeDf2 = minMapeDf2.merge(weekDf2, on=['media', 'country','max_r'], how='left')
    minMapeDf2 = minMapeDf2[minMapeDf2['mape_revenue'] == minMapeDf2['minMape']]
    minMapeDf2 = minMapeDf2.drop_duplicates(subset=['media', 'country','max_r'])

    # 合并日MAPE
    minMapeDf2 = minMapeDf2.merge(dayDf2, on=['media', 'country', 'group_name','max_r'], suffixes=('_week', '_day'))

    # 选择需要的列
    resultDf = minMapeDf2[['media', 'country', 'group_name', 'minMape', 'mape_revenue_day','max_r']]
