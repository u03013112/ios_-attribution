...
def createTable():
    if 'o' in globals():
        ...
        table_name = 'lastwar_predict_day1_pu_pct_by_cost_pct__nerf_r_configurations'
...
SELECT
    ...
    MAX(
        CASE
            WHEN percentile_rank = 90 THEN revenue_1d
        END
    ) AS p90_revenue_1d,
    MAX(
        CASE
            WHEN percentile_rank = 95 THEN revenue_1d
        END
    ) AS p95_revenue_1d,
    MAX(
        CASE
...
FROM
    ...
    p90 = data['p90_revenue_1d'].values[0]
    p95 = data['p95_revenue_1d'].values[0]
    ...
    configurations = [
        ...
        {
            'group_name': 'g3__percentile90_95',
            'payUserGroupList': [
                {'name': '0_90', 'min': 0, 'max': p90},
                {'name': '90_95', 'min': p90, 'max': p95},
                {'name': '95_inf', 'min': p95, 'max': 1e10}  # 将 np.inf 替换为 1e10
            ]
        }
    ]

    maxRList = [200,400,600,800,1000,1e10]
    nerfConfigurations = []
    for maxR in maxRList:
        for config in configurations:
            newConfig = config.copy()
            # newConfig['group_name'] = newConfig['group_name'] + f'__nerfR{maxR}'
            newConfig['max_r'] = maxR
            nerfConfigurations.append(newConfig)
            
    return nerfConfigurations
