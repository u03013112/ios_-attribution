...
def main(dirFilePath):
    ...
    addText(tenantAccessToken,docId,'',f'本文档每天发布，针对里程碑相关数据，数据均是满7日数据。\n使用可获得的最近的两周的满7日数据。并分为本周期与上周期两周进行环比。请注意周期中提到的具体日期。')

    addHead1(tenantAccessToken,docId,'','里程碑')
    with open(os.path.join(dirFilePath,'report1Text_1.txt'),'r') as f:
        text = f.read()
        addText(tenantAccessToken,docId,'',text,bold=True)

    with open(os.path.join(dirFilePath,'report1Text_2.txt'),'r') as f:
        text = f.read()
        addText(tenantAccessToken,docId,'',text)

    with open(os.path.join(dirFilePath,'report1Text_3.txt'),'r') as f:
        text = f.read()
