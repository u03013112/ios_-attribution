...
def createTable1():
    if 'o' in globals():
        ...
        table = o.create_table('lastwar_lcb_pic1', schema, if_not_exists=True)
        return table
    else:
...
def createTable2():
    if 'o' in globals():
        ...
        schema = Schema(columns=columns, partitions=partitions)
        table = o.create_table('lastwar_lcb_pic2', schema, if_not_exists=True)
        return table
    else:
        print('createTable failed, o is not defined')
...
def writeTable(df, dayStr, tableName):
    ...
    if 'o' in globals():
        ...
        print(dayStr)
        print(df)
        df.to_csv('/src/data/lastwar_lcb_pic1.csv', index=False)
...
def main():
    ...
    for day in startdayList:
        deletePartition(day, 'lastwar_lcb_pic1')
        deletePartition(day, 'lastwar_lcb_pic2')

    for (startday, endday), group in totalDf.groupby(['startday', 'endday']):
        pic1Df = group[['startday','endday','install_day', 'cost', 'sum_cost', 'target_usd']].copy()
        # write to DB
        startdayList = pic1Df['startday'].unique()
        for day in startdayList:
            pic1Df2 = pic1Df[pic1Df['startday'] == day]
            writeTable(pic1Df2, day, 'lastwar_lcb_pic1')

    for (platform, country, startday, endday), group in result_df.groupby(['platform', 'country', 'startday', 'endday']):
        pic2Df = group[['platform','country','startday','endday','install_day','cost','sum_cost','sum_d7roi','target_d7roi']].copy()
        # write to DB
        startdayList = pic2Df['startday'].unique()
        for day in startdayList:
            pic2Df2 = pic2Df[pic2Df['startday'] == day]
            writeTable(pic2Df2, day, 'lastwar_lcb_pic2')
