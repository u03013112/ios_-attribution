# 针对lastwar的融合归因
# 采用skan的原始表，只归因至媒体层级
# 与之前的归因至媒体层级区别，Facebook添加了使用Facebook API获取skan_campaign_id与campaign_id&campaign_name的对应关系
# 所有DB表都在原表名后添加后缀“_fb_20240919”
...
def getFacebookAccessToken():
    ...
    sql = '''
        select k,v from j_fb_config
        where k = 'access_token';
    '''
    df = execSql(sql)
    token = df[df['k'] == 'access_token']['v'].values[0]
    token = token + 'ByZBu'
...
def getSkanCampaignId2(minValidInstallTimestamp, maxValidInstallTimestamp):
    # 将时间戳转换为日期字符串
    minValidInstallDayStr = datetime.fromtimestamp(minValidInstallTimestamp,timezone.utc).strftime('%Y-%m-%d')
    maxValidInstallDayStr = datetime.fromtimestamp(maxValidInstallTimestamp,timezone.utc).strftime('%Y-%m-%d')
    df = getSkanCampaignId(since=minValidInstallDayStr, until=maxValidInstallDayStr)

    df = df.groupby(['day', 'skan_campaign_id']).agg({
        'campaign_id': 'first',
        'campaign_name': 'first'
    }).reset_index()
    ...
    # df的day从类似“2024-08-01”转换为“20240801”
...
def meanAttributionFastv2(userDf, skanDf):
    ...
    userDf = userDf.groupby(['cv', 'install_timestamp','install_date','country']).agg({'customer_user_id': lambda x: '|'.join(x),'count': 'sum'}).reset_index()
    ...
    skanDf = skanDf.groupby(['media','cv', 'min_valid_install_timestamp', 'max_valid_install_timestamp','usd','day','country']).agg({'count': 'sum'}).reset_index(drop = False)
    skanDf['usd x count'] = skanDf['usd'] * skanDf['count']

    # 排序，将country == 'Other'的放在最后，这样在分配时，会优先匹配country
    skanDf0 = skanDf.loc[skanDf['country'] == 'Other']
    skanDf1 = skanDf.loc[skanDf['country'] != 'Other']
    skanDf = pd.concat([skanDf1, skanDf0], ignore_index=True)
    ...
    for i in range(N):  
        ...
        for index, item in tqdm(skanDf_to_process.iterrows(), total=len(skanDf_to_process)):
            ...
            condition_country = userDf['country'] == itemCountry
    ...
    writeSkanToDB(skanFailedDf,'lastwar_ios_rh_skan_raw_fb_20240919_failed')
...
def createTable():
    if 'o' in globals():
        ...
        table = o.create_table('lastwar_ios_funplus02_adv_uid_mutidays_media_fb_20240919', schema, if_not_exists=True)
        return table
    else:
        print('createTable failed, o is not defined')

# 将处理好的skan存入表中
def createSkanTable(table_name = 'lastwar_ios_rh_skan_raw_fb_20240919'):
    if 'o' in globals():
        columns = [
            Column(name='media', type='string', comment='media,like Facebook Ads'),
...
def deleteTable(dayStr):
    ...
    if 'o' in globals():
        t = o.get_table('lastwar_ios_funplus02_adv_uid_mutidays_media_fb_20240919')
...
def writeSkanTable(df1,dayStr,table_name = 'lastwar_ios_rh_skan_raw_fb_20240919'):
    df = df1.copy()
    # 格式整理
...
def main():
    ...
    skanDf['minValidInstallDay'] = pd.to_datetime(skanDf['min_valid_install_timestamp'], unit='s').dt.strftime('%Y%m%d')
    skanCampaignDf = getSkanCampaignId2(minValidInstallTimestamp, maxValidInstallTimestamp)
    skanCampaignDf['media'] = 'Facebook'
    skanCampaignDf.rename(columns={
        'day': 'minValidInstallDay',
        'skan_campaign_id': 'skad_campaign_id'
    }, inplace=True)

    print('len skanCampaignDf:',len(skanCampaignDf))
    print(skanCampaignDf.head(5))

    print('skanDf.dtypes:',skanDf.dtypes)
    print('skanCampaignDf.dtypes:',skanCampaignDf.dtypes)

    skanDf['skad_campaign_id'] = skanDf['skad_campaign_id'].fillna('0.0')
    skanDf['skad_campaign_id'] = skanDf['skad_campaign_id'].astype('str')
    skanCampaignDf['skad_campaign_id'] = skanCampaignDf['skad_campaign_id'].astype('str')

    skanDf = skanDf.merge(skanCampaignDf[['minValidInstallDay','media','skad_campaign_id','country']],on=['minValidInstallDay','media','skad_campaign_id'],how='left')
    skanDf['country'] = skanDf['country'].fillna('Other')
    ...
    countryGroupDf = getCountryGroupList2()
    countryGroupDf.rename(columns={'country':'country_code'},inplace=True)
    countryGroupDf.rename(columns={'countrygroup':'country'},inplace=True)
    afDf = afDf.merge(countryGroupDf,on='country_code',how='left')
    afDf['country'] = afDf['country'].fillna('Other')
    ...
    # 将skanDf2存档
    skanDf['count'] = 1
    writeSkanToDB(skanDf,'lastwar_ios_rh_skan_raw_fb_20240919')
...
init()
createTable()
createSkanTable('lastwar_ios_rh_skan_raw_fb_20240919')
createSkanTable('lastwar_ios_rh_skan_raw_fb_20240919_failed')
