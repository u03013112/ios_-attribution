...
# 修改线上版本代码
# 1.在获取SKAN的时候加入媒体维度，并且一直维持在待分配的SKAN中
# 2.在融合归因的最后，加入分配失败的skan记录，记载表中。要记录skan的尽量多的信息，比如postback时间，有效时间范围等，方便之后查询。
# 3.有必要的话可以顺便记录目前campaign id 与media的关系，这个可能其他表里有，但是这个我可以留着自己用。把每天涉及到的campaign id 与media的关系记录下来。
# 4.最后再做一个可以查询一段时间内分配失败的skan，包括条目，和金额。最好也是分campaign，media的，之后可以自行汇总。再和skan记录表一起，计算比例。
