...
def createAosCohortCostRevenueDailyForCheckView():
	sql = """
CREATE OR REPLACE VIEW lw_20250822_aos_cohort_cost_revenue_day_forcheck_view_by_j AS
SELECT
	case
		when roi.app_package in ('com.fun.lastwar.gp','com.fun.lastwar.vn.gp') then 'com.fun.lastwar.gp'
	end AS app_package,
...
def createIosCohortCostRevenueDailyForCheckView():
	sql = """
CREATE OR REPLACE VIEW lw_20250822_ios_cohort_cost_revenue_day_forcheck_view_by_j AS
SELECT
	case
		when roi.app_package in ('id6448786147', 'id6736925794') then 'id6448786147'
	end AS app_package,
...
def createCohortCostRevenueDailyForCheckView():
	sql = """
CREATE OR REPLACE VIEW lw_20250822_cohort_cost_revenue_day_forcheck_view_by_j AS
SELECT * FROM lw_20250822_aos_cohort_cost_revenue_day_forcheck_view_by_j
UNION ALL
SELECT * FROM lw_20250822_ios_cohort_cost_revenue_day_forcheck_view_by_j
;
	"""
	print(f"Executing SQL: {sql}")
	execSql2(sql)
...
def createCohortCostRevenueWeeklyForCheckView():
	# 将 @`data_science`.`default`.`lw_20250822_cohort_cost_revenue_day_forcheck_view_by_j` 按周聚合，
	# 要求添加两列 monday,sunday,其中monday作为周的开始，sunday作为周的结束，均使用格式 'yyyyMMdd'。
	# 按照app_package，country_group，mediasource，ad_type，monday，sunday进行分组，分组内所有数据SUM
	# 只要满日数据，不满日的收入 为NULL
	sql = """
CREATE OR REPLACE VIEW lw_20250822_cohort_cost_revenue_weekly_forcheck_view_by_j AS
with calculated_dates as (
	select
		app_package,
		country_group,
		...
		revenue_d150
	from
		`data_science`.`default`.`lw_20250822_cohort_cost_revenue_day_forcheck_view_by_j`
)
select
	app_package,
	country_group,
...
FROM milestone m
LEFT JOIN lw_20250822_cohort_cost_revenue_weekly_forcheck_view_by_j j
...
def main():
	...
	createCohortCostRevenueDailyForCheckView()
	createCohortCostRevenueWeeklyForCheckView()
