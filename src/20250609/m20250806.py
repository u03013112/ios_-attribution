...
def bayesian_fit_media_coefficients(df, country, mediaList,mediaOtherList):
    ...
    organicRevenueConfigList = [
        {'ratio': 0.1, 'mu': totalRevenueMean * 0.1, 'sigma': totalRevenueMean * 0.001},
        {'ratio': 0.2, 'mu': totalRevenueMean * 0.2, 'sigma': totalRevenueMean * 0.001},
        {'ratio': 0.3, 'mu': totalRevenueMean * 0.3, 'sigma': totalRevenueMean * 0.001}
    ]
...
def create_result_table():
    ...
    # 定义分区（tag作为分区字段）
    partitions = [
        Partition(name='tag', type='string', comment='标签分区，格式：20250806_{organic_ratio}')
    ]
...
def write_results_to_odps(allResultsDf):
    ...
    try:
        ...
        for organic_ratio, group_df in tag_groups:
            ...
            tag = f"20250806_{organic_ratio_str}"
...
def main():
    ...
    # 主要分析并拟合的媒体
    mediaList = ['Facebook Ads','moloco_int','bytedanceglobal_int']
    # 剩余媒体
