        ...
        SELECT
            ...
            COALESCE(
                sum(purchases.event_revenue_usd) FILTER (
                    WHERE
                        datediff(
                            to_date(purchases.event_time, "yyyy-mm-dd hh:mi:ss"),
                            to_date(purchases.install_time, "yyyy-mm-dd hh:mi:ss")
                        ) <= 6
                ),
                0
            ) AS r7usd,
...
def main():
    ...
    afDf = afDf.groupby(['install_date','media_source']).sum().reset_index()
