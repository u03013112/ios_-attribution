...
def main():
    ...
    for adType in adTypeList:
        print(adType)
        adTypeDf = df[df['ad type']==adType].copy(deep=True)
        adTypeDf['campaign_id'] = adTypeDf['campaign_id'].astype(int).astype(str)
        ...
        sortDf = adTypeDf[['r7usd7','impressions7','clicks7','installs7','cost7']].sort_values(by=['r7usd7'], ascending=False).reset_index()
        # 画图，将r7usd7,impressions7,clicks7,installs7,cost7画在一张图上
        fig, ax = plt.subplots(figsize=(16, 6))
        ax.plot(sortDf['r7usd7']*10, label='r7usd7')
        # ax.plot(sortDf['impressions7'], label='impressions7')
        # ax.plot(sortDf['clicks7'], label='clicks7')
        # ax.plot(sortDf['installs7'], label='installs7')
        ax.plot(sortDf['cost7'], label='cost7')
        ax.legend()
        plt.title(adType)
        plt.savefig('/src/data/zk2/mmm_'+adType+'.png')
        plt.close()
