...
def makeCvMap(levels):
    ...
    l = len(mapData['cv'])
    mapData['max_event_revenue'][l-1] = mapData['min_event_revenue'][l-1]
    mapData['avg'][l-1] = mapData['min_event_revenue'][l-1]
...
def addCv(userDf,cvMapDf,usd='r1usd',cv='cv'):
    ...
    userDfCopy.loc[userDfCopy[usd]>max,'avg'] = avg
    return userDfCopy

def check(userDf, cvMapDf,usd='r1usd'):
    df = addCv(userDf, cvMapDf,usd=usd)
    # print(df[df['cv']>0].head(10))
    dfGroup = df.groupby('install_date').sum().reset_index()
...
def main2():
    ...
    userDf = userDf[(userDf['install_date'] >= '2023-05-01')
        & (userDf['install_date'] <= '2023-07-31')                
    ]
    levels = makeLevels1(userDf,usd='r2usd',N=32)
    cvMapDf = makeCvMap(levels)
    cvMapDf.to_csv('/src/data/zk2/iosr2UsdMap2.csv', index=False)
    mape = check(userDf, cvMapDf,usd='r2usd')
    print(mape)
...
def main3():
    ...
    userDf = userDf[(userDf['install_date'] >= '2023-02-01')
        & (userDf['install_date'] <= '2023-07-31')                
    ]
    levels = makeLevels1(userDf,usd='r2usd',N=32)
    cvMapDf = makeCvMap(levels)
    cvMapDf.to_csv('/src/data/zk2/iosr2UsdMap3.csv', index=False)
    mape = check(userDf, cvMapDf,usd='r2usd')
    print(mape)
