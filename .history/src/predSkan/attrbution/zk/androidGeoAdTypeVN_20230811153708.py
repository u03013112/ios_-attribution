...
def main24Adv(fast = False,onlyCheck = False):
    ...
    if onlyCheck == False:
        if fast == False:
            ...
            userLevelDf = pd.read_csv(getFilename('userLevelVn'))
            userLevelDf = userLevelDf.groupby(['uid']).agg({'level':'max'}).reset_index()
...
# def debug():
#     userLevelVnDf = pd.read_csv(getFilename('userLevelVn'))
#     # 检查userLevelVnDf中是不是有重复的uid
#     print('userLevelVnDf len:',len(userLevelVnDf))
#     print('userLevelVnDf uid len:',len(userLevelVnDf['uid'].unique()))
