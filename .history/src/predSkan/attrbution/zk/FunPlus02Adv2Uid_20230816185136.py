...
def delTable(dayStr):
    sql = f'''ALTER TABLE topwar_ios_funplus02_adv_uid DROP IF EXISTS PARTITION (day = '{dayStr}');'''
    print(sql)
    execSql(sql)
    return
...
def main(dayStr):
    ...
    sql = f'''
        INSERT INTO
            rg_bi.topwar_ios_funplus02_adv_uid PARTITION (day='{dayStr}')
        SELECT
            e.customer_user_id,
            a.install_date,
            a.`facebook ads rate` as facebook_ads_rate,
            a.`googleadwords_int rate` as googleadwords_int_rate,
            a.`bytedanceglobal_int rate` as bytedanceglobal_int_rate
        FROM
