...
def meanAttribution(userDf, skanDf):
    ...
    userDf['count'] = 1
    print("Before groupby:", userDf.columns)
    userDf = userDf.groupby(['cv', 'country_code', 'install_timestamp','install_date']).agg({'customer_user_id': lambda x: '|'.join(x),'count': 'sum'}).reset_index(drop = False)
    print("After groupby and reset_index:", userDf.columns)
